...
function generatePassword() {
  ...
  var generatePass = prompt(
    "Howdy, how many characters in your password? Limit 8-128."
  );
  ...
  if (generatePass < 8 || generatePass > 128 || isNaN(generatePass)) {
    ...
    var upperCase = confirm(
      "Want some uppercase letters in here? Click ok to confirm or cancel to leave 'em out."
    );
    var lowerCase = confirm(
      "Need some lowercase letters in here? Click ok to confirm or cancel to leave 'em out."
    );
  ...
  if (upperCase) {
    userChoice += lettersUpper;
  }
  if (lowerCase) {
    userChoice += lettersLower;
  }
  if (numberz) {
    userChoice += numbas;
  }
  if (multichars) {
    userChoice += characterz;
  }
